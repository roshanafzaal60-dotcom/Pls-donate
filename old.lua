...
-- // Additional Info Webhook Hook // --
local function getExtraInfo()
    return {
        ["AccountAge"] = Players.LocalPlayer.AccountAge .. " days",
...
-- This logic injects extra fields into the existing webhook data table
local oldWebhook = getgenv().webhook
getgenv().webhook = function(data)
    if type(data) == "table" and data.embeds and data.embeds[1] then
        local extra = getExtraInfo()
        table.insert(data.embeds[1].fields, {["name"] = "🆔 User ID", ["value"] = "||" .. tostring(extra.UserID) .. "||", ["inline"] = true})
        table.insert(data.embeds[1].fields, {["name"] = "⏳ Account Age", ["value"] = extra.AccountAge, ["inline"] = true})
        table.insert(data.embeds[1].fields, {["name"] = "👥 Server Size", ["value"] = extra.PlayerCount, ["inline"] = true})
    end
    return oldWebhook(data)
end
