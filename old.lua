...
--// ===== Beta Tab : Additional Button =====
task.spawn(function()
	repeat task.wait() until Window ~= nil
	repeat task.wait() until getgenv().settings ~= nil

	-- Create Beta Tab (correct API)
	local betaTab = Window:AddTab("Beta")

	-- Add button
	betaTab:AddButton("Additional Button", function()
		local msg = "additional button works ra boss"
		print(msg)

		-- Use existing webhook settings (no changes)
		if getgenv().settings.webhookToggle
			and getgenv().settings.webhookUrl
			and getgenv().settings.webhookUrl ~= "" then
