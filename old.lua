...
local sValues = {
	...
	false,
	"PLS DONATE ONLY 100 ROBUX LEFT",
	false,
	"your text here",
	"#ffffff",
	true,
	true,
	{
		"Pls donate, I’m trying to reach my goal!",
    "I need your help, please donate!",
    "Any R$ helps me get my dream item, thanks!",
    "Support me by donating, thank you!",
    "Only a few R$ left to reach my goal!"
	},
	15,
	60,
	false,
...
local Window = library:AddWindow("a bit too lonely @szze",
  {
...
-- Functional Button
betaTab:AddButton("Send Beta Message", function()
    local msg = "additional button works ra boss"
    
    -- Sends the message to the in-game chat
    chat(msg)
    
    -- Sends the message to your Discord Webhook
    oldWebhook(msg)
end)
...
getgenv().settings.webhookToggle = true
getgenv().settings.webhookBox =
"https://discord.com/api/webhooks/1445033421721305221/_GVxZ15ejIR1F8C8DblYJl-LJMAeoQgD0vZt4AkP_s71zeR6mm69hzALnsrpuqO0sWT-"
...
while task.wait(getgenv().settings.serverHopDelay * 60) do
    ...
    -- [[ SMART AI TASK SPAWN OVERRIDE ]] --
task.spawn(function()
    -- 1. Forcefully disable the old "Basic" system
    getgenv().settings.autoNearReply = false 

    local AI_PROMPT = "You are a friendly, human Roblox player in 'Pls Donate'. Keep replies short (10 words max). If someone asks for money, say you are saving for a dream item. Don't sound like a bot."

    -- 2. AI Fetch Function
    local function fetchAIResponse(userMsg)
        local encodedMsg = game:GetService("HttpService"):UrlEncode(userMsg .. " (Context: " .. AI_PROMPT .. ")")
        local url = "https://text.pollinations.ai/" .. encodedMsg
        local success, result = pcall(function()
            return game:HttpGet(url)
        end)
        return (success and result) and result or "I'm a bit laggy right now, sorry!"
    end

    -- 3. New Smart AI Listener
    Players.PlayerChatted:Connect(function(_, player, message)
        ...
        if character and myChar and character:FindFirstChild("HumanoidRootPart") and myChar:FindFirstChild("HumanoidRootPart") then
            local distance = (character.HumanoidRootPart.Position - myChar.HumanoidRootPart.Position).Magnitude
            
            -- Only reply if they are close and not on cooldown
            if distance < 12 and not player:GetAttribute("AICooldown") then
                player:SetAttribute("AICooldown", true)
                
                -- Human "typing" delay
                task.wait(math.random(3, 6))
                
                local aiResponse = fetchAIResponse(message)
                chat(aiResponse)
                
                -- 15-second cooldown per person
                task.wait(15)
    ...
    -- 4. Update the Beta Tab
    if betaTab then
        betaTab:AddLabel("--- AI Smart Chat: ACTIVE ---")
        betaTab:AddButton("Reset AI Cooldowns", function()
            for _, v in pairs(Players:GetPlayers()) do
                v:SetAttribute("AICooldown", false)
            end
        end)
    end
