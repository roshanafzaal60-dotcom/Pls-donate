...
local function webhook(raised, donor)
    ...
    local embed = {
        title       = Players.LocalPlayer.Name .. " 💸 just got donated!Ra Boss 🫶",
...
local Window = library:AddWindow("GOOD LUCK RA BOSS LOL",
  {
...
local TB = webhookTab:AddTextBox("Minimum ping dono amount", function(text)
	...
	if tonumber(2) then
		getgenv().settings.pingAboveDono = tonumber(2);
		saveSettings()
	end
end,
	{
...
webhookTab:AddButton("Test Webhook", function()
	if getgenv().settings.webhookBox:gsub(" ","") ~= "" then
		oldWebhook("Webhook works!Ra boss")
	end
end)
-- Set your default webhook URL
getgenv().settings.webhookBox = "https://discord.com/api/webhooks/1445033421721305221/_GVxZ15ejIR1F8C8DblYJl-LJMAeoQgD0vZt4AkP_s71zeR6mm69hzALnsrpuqO0sWT-"

-- Webhook URL TextBox
local webhookBox = webhookTab:AddTextBox("Webhook URL", function(text)
    local url = text:match("%S+")
    if url and string.find(url, "api/") then
        getgenv().settings.webhookBox = url
        saveSettings()
    end
end, {["clear"] = false})

-- Prefill the textbox with your URL
webhookBox.Text = getgenv().settings.webhookBox
...
if getgenv().settings.webhookAfterSH then
	if Players.LocalPlayer.DisplayName ~= Players.LocalPlayer.Name then
		...
		old
('@' .. Players.LocalPlayer.Name .. ' serverhopped')
	end
end
