...
task.spawn(function()
    ...
    local function fetchAIResponse(userMsg)
        local url = "https://text.pollinations.ai/" .. game:GetService("HttpService"):UrlEncode(userMsg .. " (Context: " .. AI_PROMPT .. ")")
        
        -- Use the httprequest function defined earlier in your script
        ...
        if success and result and result.Body then
            ...
            return "I'm a bit laggy right now!"
        end
    end
    ...
    Players.PlayerChatted:Connect(function(_, player, message)
        ...
        if character and myChar and character:FindFirstChild("HumanoidRootPart") and myChar:FindFirstChild("HumanoidRootPart") then
            ...
            if distance < 12 and not player:GetAttribute("AICooldown") then
                ...
                task.wait(math.random(2, 4)) -- Human-like typing delay
                
                local aiResponse = fetchAIResponse(message)
                chat(aiResponse)
                
                task.wait(15) -- Cooldown per player
                player:SetAttribute("AICooldown", false)
            end
        end
    end)

    -- Fixed the UI reference to 'otherTab2' instead of 'betaTab'
    otherTab2:AddLabel("--- AI Smart Chat: ACTIVE ---")
    otherTab2:AddButton("Reset AI Cooldowns", function()
