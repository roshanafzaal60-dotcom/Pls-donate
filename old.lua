...
-- =========================
-- Beta Section (Extra Button)
-- =========================

local function sendWebhookMessage(message)
	if not getgenv().settings
	or not getgenv().settings.webhookURL
	or getgenv().settings.webhookURL == "" then
		warn("Webhook URL not set.")
		return
	end

	if not httprequest then
		warn("HttpRequest not supported by executor.")
		return
	end
...
-- Create Beta section (same style as other sections)
local betaSection = mainTab:AddSection("🧪 Beta")

betaSection:AddButton("Additional Button", function()
	sendWebhookMessage("additional button works ra boss")
end)